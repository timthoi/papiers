...
* @subpackage	Classes
*/
class PapiersdefamillesClassModelItem extends JModelAdmin
{
	/**
	* Data array
	*
	...
	*/
	public function canCheck($record)
	{
		...
		{			
			$this->setError(JText::_("PAPIERSDEFAMILLES_TASK_RESULT_THE_USER_CHECKING_OUT_DOES_NOT_MATCH_THE_USER_WHO_CHECKED_OUT_THE_ITEM"));
			return false;			
		}
	...
	public function deletefiles($pks, $fileFields)
	{
		...
		foreach($fileFields as $fieldName => $op)
		{
			...
			foreach($files as $fileObj)
			{
				...
				if (!PapiersdefamillesHelperFile::removeFile($imagePath, $op))
					$errors = true;
			}
		}
	...
	*/
	...
	{
		$tbl = '#__';
		$com = 'PAPIERSDEFAMILLES_';

		switch($fieldname)
		{
			case 'subscriptionplan': return $tbl.$com. 'subscriptionplans';
			case 'departure_city_id': return $tbl.$com. 'typedocuments';
			case 'continent_id': return $tbl.$com. 'subscriptionplans';
			case 'country_id': return $tbl.$com. 'categories';
			case 'city_id': return $tbl.$com. 'cities';
			case 'created_by': return $tbl. 'users';
			case 'modified_by': return $tbl. 'users';
			case 'ticket_type_id': return $tbl.$com. 'documents';
		}
	}
	...
	*/
	...
	{
		$model = CkJModel::getInstance(ucfirst($table), 'PapiersdefamillesModel');
	...
	*/
	public function getForm($data = array(), $loadData = true, $control = 'jform')
	{
		...
		$form->addRulePath(JPATH_ADMIN_PAPIERSDEFAMILLES . '/models/rules');
	...
	*/
	...
	{
		$modelList = CkJModel::getInstance($this->getNameList(), 'PapiersdefamillesModel');
		if ($modelList)
	...
	*/
	protected function loadForm($name, $source = null, $options = array(), $clear = false, $xpath = false)
	{
		...
		// Get the form.
		JForm::addFormPath(JPATH_PAPIERSDEFAMILLES . '/models/forms');
		JForm::addFieldPath(JPATH_PAPIERSDEFAMILLES . '/models/fields');
	...
	*/
	public function loadXref(&$item, $objectField, $xrefTable, $on, $key, $states = array(), $context = 'object.default')
	{
		...
		{
			$model = CkJModel::getInstance($xrefTable, 'PapiersdefamillesModel');
	...
	*/
	protected function populateState()
	{
		...
		if (defined('JDEBUG'))
			$_SESSION["Papiersdefamilles"]["Model"][$this->getName()]["State"] = $this->state;
	...
	*/
	...
	{
		$baseFolder = JPATH_PAPIERSDEFAMILLES . '/fork/models/forms';
	...
	*/
	public function toggle($fieldName, $pk = null, $max = 1)
	{
		...
		if (!$result)
		{
			JError::raiseWarning(1106, JText::sprintf("PAPIERSDEFAMILLES_MODEL_IMPOSSIBLE_TO_TOGGLE", $fieldName));
			return false;
		}
			...
			switch($field->type)
			{
				...
				case 'ckcalendar':
					...
					if ($value && (string)$field->format)
					{
						...
						if (!is_numeric($value) && !PapiersdefamillesHelperDates::isNull((string)$value))
						{
							$time = PapiersdefamillesHelperDates::getSqlDate($value, array($field->format));

							if ($time === null){
								JError::raiseWarning(1203, JText::sprintf('PAPIERSDEFAMILLES_VALIDATOR_WRONG_DATETIME_FORMAT_FOR_PLEASE_RETRY', $field->label));
								$valid = false;
							}
							else
