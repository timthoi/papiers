...
/**
 * Papiersdefamilles Auction Controller
 *
 * @package	Papiersdefamilles
 * @subpackage	Classes
 * @since	v.1.0
 */
class PapiersdefamillesModelDistricts extends PapiersdefamillesClassModelList
{
	/**
	...
	protected function prepareQuery(&$query)
	{
		...
		{
			case 'districts.default':

				break;

			case 'districts.filter':

				$this->setState('list.limit', null);
				$this->setState('list.start', null);
				break;

			case 'districts.modal':
		...
		// WHERE - SEARCH : searchSearch : search on name
		$searchSearch = $this->getState('search.search');
		$this->addSearch('search', 'a.name', 'like');

		if (($searchSearch != '') && ($searchSearchVal = $this->buildSearch('search', $searchSearch)))
		{
			$this->addWhere($searchSearchVal);
		}
