...
* @subpackage	Classes
*/
class PapiersdefamillesModelCity extends PapiersdefamillesClassModelItem
{
	/**
	* View list alias
	*
	...
	*/
	public function getTable($type = 'city', $prefix = 'PapiersdefamillesTable', $config = array())
	{
	...
	*/
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_papiersdefamilles.edit.city.data', array());
		...
		if (empty($data)) {
			...
			if ($this->getState('city.id') == 0)
			{
				...
				$data->name = null;
				$data->alias = null;
				$data->continent_id = $jinput->get('filter_continent', $this->getState('filter.continent'), 'INT');
				$data->country_id = $jinput->get('filter_country', $this->getState('filter.country'), 'INT');
				$data->published = null;
				$data->ordering = null;
