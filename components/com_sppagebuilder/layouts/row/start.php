...
if(!$fluid_row){
	$html .= '<section id="' . $row_id . '" class="sppb-section ' . $custom_class . '" '.$addon_attr . $parallax_params .'>';
	...
	$html .= '<div id="' . $row_id . '" class="sppb-section ' . $custom_class . '" '.$addon_attr . $parallax_params .'>';
...
if(isset($options->background_type)){
	if (!empty($external_video) && $options->external_background_video && $options->background_type == 'video') {
		...
		switch($video['host']) {
			case 'youtu.be':
			$id = trim($video['path'],'/');
			$src = '{videoURL:\''.$id.'\',containment:\''.$vidId.'\',autoPlay:true,mute:true,startAt:0,opacity:1,loop:'.$ext_video_loop.',showControls:false,stopMovieOnBlur:false,showYTLogo:false,gaTrack:false,rel:0}';
			break;
			...
			case 'www.vimeo.com':
			$id = trim($video['path'],'/');
			$vimeoSrc = '{videoURL:\''.$id.'\',containment:\''.$vidId.'\',autoPlay:true,mute:true,startAt:0,opacity:1,loop:'.$ext_video_loop.',showControls:false,stopMovieOnBlur:false,showYTLogo:false,gaTrack:false}';
		}
	...
	if (!empty($external_video) && $options->external_background_video && $options->background_video) {
		...
		switch($video['host']) {
			...
			parse_str($video['query'], $query);
			$id = $query['v'];
			$src = '{videoURL:\''.$id.'\',containment:\''.$vidId.'\',autoPlay:true,mute:true,startAt:0,opacity:1,loop:'.$ext_video_loop.',showControls:false,stopMovieOnBlur:false,showYTLogo:false,gaTrack:false}';
			break;
			case 'vimeo.com':
			case 'www.vimeo.com':
			$id = trim($video['path'],'/');
