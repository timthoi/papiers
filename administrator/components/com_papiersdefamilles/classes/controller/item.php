...
/**
* Papiersdefamilles  Controller
*
* @package	Papiersdefamilles
* @subpackage	
*/
class PapiersdefamillesClassControllerItem extends JControllerForm
{
	/**
	...
	* @var string
	*/
	protected $text_prefix = 'com_papiersdefamilles';
	...
	*/
	public function add()
	{
		...
		$this->applyRedirection($result, array(
			'com_papiersdefamilles.' . $this->view_list . '.default',
			'com_papiersdefamilles.' . $this->view_item . '.' . $this->getLayout('edit')
		));
	}
	...
	*/
	protected function applyRedirection($result, $redirections, $vars = array())
	{
		...
		switch($redirection)
		{
			...
			case 'stay':
				$this->setRedirect(PapiersdefamillesHelper::urlRequest($vars));
				return;
				break;
		...
		// FILTERs followers
		$model = CkJModel::getInstance($this->view_list, 'PapiersdefamillesModel');
		if ($model)
		{
	...
	*/
	public function delete()
	{
		...
		else
		{
			...
			{
				$result = true;
				$this->setMessage(JText::plural('PAPIERSDEFAMILLES_ITEMS_SUCCESSFULLY_DELETED', count($cid)));
			}
			else
			{
	...
	public function redirect()
	{
		if ($this->redirect)
		{
			...
			if ($jinput->get('return') == 'json')
			{
				...
				PapiersdefamillesClassAjax::responseJson(array(
	...
	*/
	protected function updateFileField($model, $fieldName, $options = array())
	{
		...
		try
		{
			$changedValue = PapiersdefamillesHelperFile::updateFile($model, $fieldName, $options, $uploadDirectory);
		}
		catch(Exception $e)
		{
