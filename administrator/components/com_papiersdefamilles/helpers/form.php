...
* @package	Papiersdefamilles
*/
class PapiersdefamillesHelperForm
{
	/**
	...
	*/
	public static function getElementGroups($element)
	{
		...
		$model = PapiersdefamillesHelper::componentModel((string)$element['model'], false);
		...
		foreach ($element->children() as $option)
		{
			...
			if ($model)
			{
				...
				$group->model = (strpos($relation->model, '.')?'':'papiersdefamilles.') . $relation->model;

				$model = PapiersdefamillesHelper::componentModel($relation->model, false);
			}
			...
			// Label Key
			$group->alias = papiersdefamillesClassModelOrm::tableFieldAlias($group->on, false);
	...
	*/
	public static function getModel($field, $item = false)
	{
		...
		return PapiersdefamillesHelper::componentModel($model, $item);
	}
