...
* @subpackage	Classes
*/
class PapiersdefamillesClassModelList extends JModelList
{
	/**
	* Data array
	*
	...
	*/
	...
	{
		$model = CkJModel::getInstance($this->view_item, 'PapiersdefamillesModel');
	...
	public function getItems()
	{
		try
		{
			...
			if ($error = $db->getErrorMsg()) {
				if (!$this->canAdmin())
					$error = JText::_('PAPIERSDEFAMILLES_ERROR_INVALID_QUERY');
	...
	*/
	...
	{
		if ($item)
			return CkJModel::getInstance($this->view_item, 'PapiersdefamillesModel');
	...
	*/
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$globalParams = JComponentHelper::getParams('com_papiersdefamilles', true);
		...
		// FILTERS
			foreach($this->filter_vars as $var => $varType)
			{
				...
				{
					$date = PapiersdefamillesHelperDates::timeFromFormat($value, $matches[1]);
					if ($date)
					{
	...
	*/
	public function updateXref($field, $values, $on, $id)
	{
		...
		$model = CkJModel::getInstance($this->getName(), 'PapiersdefamillesModel');
		...
		else
		{
			...
			{
				$query = $db->getQuery(true);
				$query->update('#__papiersdefamilles_' . $this->getName())
