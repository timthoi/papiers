...
defined('PAPIERSDEFAMILLES_UPLOAD_DEFAULT_DIR') or define("PAPIERSDEFAMILLES_UPLOAD_DEFAULT_DIR", 'COM_MEDIAS');
defined('PAPIERSDEFAMILLES_UPLOAD_DEFAULT_RENAME') or define("PAPIERSDEFAMILLES_UPLOAD_DEFAULT_RENAME", '{ALIAS}.{MIMEXT}');
defined('PAPIERSDEFAMILLES_UPLOAD_CHMOD_FOLDER') or define("PAPIERSDEFAMILLES_UPLOAD_CHMOD_FOLDER", 0754);
defined('PAPIERSDEFAMILLES_UPLOAD_CHMOD_FILE') or define("PAPIERSDEFAMILLES_UPLOAD_CHMOD_FILE", 0644);
defined('PAPIERSDEFAMILLES_IMAGES_MAX_WIDTH') or define("PAPIERSDEFAMILLES_IMAGES_MAX_WIDTH", 1000);
defined('PAPIERSDEFAMILLES_IMAGES_MAX_HEIGHT') or define("PAPIERSDEFAMILLES_IMAGES_MAX_HEIGHT", 1000);
defined('PAPIERSDEFAMILLES_IMAGES_ALLOWED_SIZES') or define("PAPIERSDEFAMILLES_IMAGES_ALLOWED_SIZES", '');
...
* @package	Papiersdefamilles
*/
class PapiersdefamillesHelperFile
{
	/**
	...
	* @var string
	*/
	protected static $extension = 'com_papiersdefamilles';
		...
		if (static::checkFileExists($dir, $file))
		{
			switch(strtolower($options["overwrite"]))
			{
				...
				case '0':
					...
					throw new \Exception(JText::sprintf( "PAPIERSDEFAMILLES_UPLOAD_EXISTS", $file->filename));
					break;
	...
	public static function getFileUrlFromElement($form, $element, $value)
	{
		...
		if (!$dir)
		{
			...
			if (!in_array($dir, array_keys($directories)))
				$dir = PAPIERSDEFAMILLES_UPLOAD_DEFAULT_DIR;
		...
		$attribs = PapiersdefamillesHelperFile::getAttributesFromElement($element);

		$url = (($method == 'indirect') && JFactory::getApplication()->isAdmin()?'administrator/':'')
			. PapiersdefamillesHelperFile::getFileUrl($indirectPath, $method, $attribs);
	...
	*/
	public static function getUploadMaxSize($string = false, $maxSizeCustom = null)
	{
		...
		if ($string)
			$maxSize = JText::sprintf("PAPIERSDEFAMILLES_UPLOAD_MAX", static::bytesToString($maxSize));
		...
		if (JFile::exists($filePath))
		{
			...
			if (in_array($remove, array('trash')))
			{
				...
				if (!JFile::move($filePath, $trashPath .'/'. JFile::getName($filePath)))
					throw new \Exception(JText::_("PAPIERSDEFAMILLES_UPLOAD_TRASH_IMPOSSIBLE"));
			...
			else if (in_array($remove, array('delete')))
			{
				if (!JFile::delete($filePath))
					throw new \Exception(JText::_("PAPIERSDEFAMILLES_UPLOAD_DELETE_IMPOSSIBLE"));
	...
	*/
	...
	{
		$pattern = (isset($options["rename"])?$options["rename"]:PAPIERSDEFAMILLES_UPLOAD_DEFAULT_RENAME);
	...
	*/
	public static function returnFile()
	{
			...
			)
		{
			...
			{
				$w = (int)min($size->width, PAPIERSDEFAMILLES_IMAGES_MAX_WIDTH);
				$h = (int)min($size->height, PAPIERSDEFAMILLES_IMAGES_MAX_HEIGHT);
				...
				if (($size != "0x0") && !in_array($size, explode(",", PAPIERSDEFAMILLES_IMAGES_ALLOWED_SIZES)))
				{
		...
		else if (!JFile::exists($filePath) || in_array($ext, $forbiddenExt))
		{
			...
			$msg = JText::sprintf( "PAPIERSDEFAMILLES_UPLOAD_FILE_NOT_FOUND", $pathNoDir);
			jexit($msg);
		}
	...
	*/
	public static function sanitizeFolder($path)
	{
		...
		@chmod($folder, PAPIERSDEFAMILLES_UPLOAD_CHMOD_FOLDER);
		if (!is_writable($folder))
			throw new \Exception(JText::sprintf( "PAPIERSDEFAMILLES_UPLOAD_READONLY_FOLDER",$path));
	...
	*/
	public static function uploadFile($dir, $uploadFile, $options = array())
	{
		...
		if (!$dir)
			throw new \Exception(JText::_("PAPIERSDEFAMILLES_UPLOAD_FOLDER_NOT_FOUND"));
		...
		if (empty($uploadFile['name']))
			throw new \Exception(JText::_("PAPIERSDEFAMILLES_UPLOAD_PLEASE_BROWSE"));
		...
		{
			throw new \Exception(JText::sprintf( "PAPIERSDEFAMILLES_UPLOAD_REFUSED_EXTENSION",
				$file->extension,
		...
		{
			throw new \Exception(JText::sprintf( "PAPIERSDEFAMILLES_UPLOAD_REFUSED_MIME",
				$file->mime,
				implode(" - ", $extensions))
			);
		}
		...
		{
			throw new \Exception(JText::sprintf( "PAPIERSDEFAMILLES_UPLOAD_REFUSED_SIZE",
		...
		if (preg_match("/\<\?php\s/", $contents))
			throw new \Exception(JText::_( "PAPIERSDEFAMILLES_UPLOAD_CONTAINS_ERRORS"));
	...
	protected static function uploadFileProcess($dirBase, $filePath, $fileTmp)
	{
		...
		if (!move_uploaded_file($fileTmp, $fileDest))
			if (!JFile::upload($fileTmp, $fileDest))
			{
				...
				throw new \Exception(JText::sprintf( "PAPIERSDEFAMILLES_UPLOAD_ERROR_UPLOAD", $file->filename));
			}


		//Protect file against execution
		@chmod($fileDest, PAPIERSDEFAMILLES_UPLOAD_CHMOD_FILE);
